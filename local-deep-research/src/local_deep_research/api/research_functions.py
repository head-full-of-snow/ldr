...
def generate_report(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        logger.info(
            "generate_report: after initial analyze_topic, "
            "strategy={}, all_links_count={}, initial_findings_links={}",
            type(system.strategy).__name__,
            len(system.all_links_of_system),
            len(initial_findings.get("all_links_of_system", [])),
        )
        ...
        # Inject sources so callers get structured link data alongside the
        # markdown content. By this point system.all_links_of_system has
        # accumulated links from both the initial research and every
        # per-section search performed by the report generator.
        report["sources"] = system.all_links_of_system
