...
def api_generate_report():
    ...
    # 注入 username 和 settings_snapshot（与 api_quick_summary 一致）
    username = get_current_username()
    if username:
        params["username"] = username
    ...
    if username:
        try:
            ...
            logger.warning(
                "Failed to get settings snapshot for generate_report")
            params["settings_snapshot"] = {}
    else:
        logger.debug("No username in session, skipping settings snapshot")
        params["settings_snapshot"] = {}
    ...
    try:
        ...
        # # Don't return the full content for large reports 将过长的报告截断
        # if (
        #     result
